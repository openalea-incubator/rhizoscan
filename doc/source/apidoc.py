# -*- coding: utf-8 -*-
"""
    (updated) Copy of the sphinx apidoc module
...
def makename(*pkg_and_mod_names): #package, module):
    """Join package, subpackage (...) and module names with a dot."""
    return '.'.join(filter(len,pkg_and_mod_names))

def cmp_file_content(fname, text):
    from itertools import izip
    
    f = file(fname,'r')
    f_content = filter(len,[line.strip() for line in f.readlines()])
    f.close()
    t_content = filter(len,[line.strip() for line in text.split('\n')])
    
    return f_content==t_content
...
def write_file(name, text, opts):
    ...
    if not opts.force and path.isfile(fname):
        if not opts.update or cmp_file_content(fname, text):
...
def create_package_file(root, master_package, subroot, py_files, opts, subs):
    ...
    # process the __init__ file
    py_file = path.splitext(py_files[0])[0]
    py_path = makename(subroot, py_file)
    heading = ':mod:`%s` Package' % makename(master_package,subroot) ##package
    text += format_heading(2, heading)
    text += format_directive(subroot or py_path, master_package)
    text += '\n'
        
    pkg_name = makename(master_package, subroot)
    subm = []
        
    # add each submodule in the package
    for py_file in py_files[1:]:
        ...
        if opts.module_page:
            ...
            create_module_file(pkg_name, py_file, opts)    
            subm.append(module)
        else:
            py_path = makename(subroot, py_file)
            heading = ':mod:`%s` Module' % py_file
            text += format_heading(2, heading)
            text += format_directive(py_path, master_package)
            text += '\n'
    ...
    if subs or subm:
        text += format_heading(2, 'Subpackages' + (' and modules' if opts.module_page else ''))
        text += '.. toctree::\n\n'
        for sub in subm:
            text += '    %s.%s\n' % (pkg_name, sub)
        for sub in subs:
...
Note: By default this script will not overwrite already created files.""")
    ...
    parser.add_option('-U', '--update', action='store_true', dest='update',
                      help='overwrite ReST files if changed only')
    ...
    parser.add_option('-M', '--module-page', action='store_true', dest='module_page',
                      help='Create a ReST file for each module')
