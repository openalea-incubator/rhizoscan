...
__all__ = ['Factory','aleanode', '_aleanode', 'wrap_module', 'wrap_package', 'create_wrap_folder']
...
class aleanode:
    ...
    Arguments of aleanode call can be either:
      ...
      3. the names (strings) of the outputs   - 'out1', 'out2'
    ...
    ------------------------
    Declaring openalea nodes using this decorator does not directly create the 
    node. It is the wrap_package() function that parse packages and modules for 
    all declared nodes, and create/update wralea file structure.
    For Package the __init__.py file must contains either declared aleanode or
    at least an empty _aleanodes_ list, for the parser to look down into its
    modules and subpackages.
    
    The setup.py files of openalea package should be calling wrap_package and
    declaring the suitable entry points at package installation
    ...
    def __call__(self,f):
        ...
        f.__setattr__('_aleanode_', self.kwargs)
...
class PFuncNode(FuncNode):
    ...
    def __init__(self, auto_caption=None, *args, **kargs):
        ...
        auto_caption: 
            ...
            Otherwise, 'all' automatically add all inputs to caption when called
                       if a number 'n', add only the nth inputs to caption
        """
    ...
    def __call__(self, inputs = ()):
        ...
        ##    self.set_caption(self.Factory.name + str(inputs[self._auto_caption]))
...
class FuncFactory(NodeFactory):
    """
    Subclass of openalea.core.NodeFactory that instanciate FuncNode specifically
    
    ##BUG: also overload get_module_node
    ##  the NodeFactory one is re-importing modules each time a node is instanciated
    """
...
def wrap_module(module, wrapper_path, attrib={}, search_path = [], test=False, verbose=True):
    ...
    for fct in module._aleanodes_:
        ...
        if not node.has_key('inputs'):
            ...
            def_val = (None,)*(c.co_argcount - len(def_val)) + def_val
...
def wrap_package(pkg, pkg_attrib={}, wrap_name=None, wrap_path=None, entry_points=None, entry_name=None, verbose=True):
    """
    Recursively wralea packages for 'pkg' and submodules/packages
    ...
    :Inputs:
        ...
        - pkg_attrib:
            ...
            These attributes take the value, if provided, following the order
              ...
              2) from pkg_attrib
              3) or the default written above
    ...
    :Outputs:
        A list of entry point string "entry-name = wrapped-package" as is used by 
        the default setup.py installer of openalea packages.           
                
    :Warning:
        package that don't contain aleanode directly (in the __init__.py file),
        must at least contain an empty __aleanodes__ list for this function to
        parse submodules and package.
          => An empty _aleanodes_ list induces recursive parsing of submodules
    """
...
def node_attribute(module, entry_name=None, parent_attrib={}):
    """
    make openalea node attributs for given module.
    
    'entry_name':    the entry_name attribut. If not given (None), use the module __name__ 

    'parent_attrib': parent package attribut, to used if module does not contains some attribut
                 
    An attribut name is choosen, first from the module attribut if it exist, then 
    in parent_attrib if it exist, or a default one is taken. 
     => Call this function with a basic module to see the default attributs - ex: make_attrib(os)
    """
...
def create_wrap_folder(path):
    ...
    Create a python wrapper folder for wralea package:
        A folder with an empty __init__.py and __wralea__.py
    ...
    :Outputs:
        file: the filename of the __wralea__.py file
        text_start: the text found before the "generated code" part ending with the start line
        text_end:   the text found after  the "generated code" part startin with the end line
        
        text_start and text_end are lists of string
    """
    
    # start and end lines of generated code
    text_start = "## ***** automatically generated code **** ##\n"
    text_end   = "## ******** end of generated code ******** ##\n"
    ...
    file = os.path.join(path,'__wralea__.py')
    if os.path.exists(file):
        ...
        start = [t.startswith(text_start) for t in text]
        if True in start:
            start = start.index(True)
            text_start = text[:start+1]
            text = text[start+1:]
        else:
            text_start = text + [text_start]
            text = []

        # find if remaining text contains text_end flag
        # if it does, store following code in text_end
        end   = [t.startswith(text_end) for t in text]
        if True in end:
            end = end.index(True) 
            text_end = text[end:]
        else:
    ...
    return file, text_start, text_end

def load_module(module_name, search_path=[]):
    """
    load module_name: a string with the name of the module as it is given to 
    import (i.e. containing package dependancies separated by '.')
    If the module (or parent package) in not in the sys path, additional 
    directories can be provided as a list of string by argument 'search_path'
    """
